...
import org.nift4.mediastorecompat.BoundsHelper.Companion.TAG
...
private object BoundsHelperApi29Impl : BoundsHelper {
    ...
    @SuppressLint("BanUncheckedReflection", "BlockedPrivateApi", "PrivateApi")
    override fun currentWindowBounds(activity: Activity): Rect {
        var bounds: Rect
        ...
        try {
            ...
            when (e) {
                ...
                is InvocationTargetException -> {
                    Log.w(TAG, "failed to load window bounds, falling back", e)
    ...
    try {
        ...
        when (e) {
            ...
            is InstantiationException -> {
                Log.w(TAG, "failed to load display cutout", e)
            }
            else -> throw e
        }
    } finally {
